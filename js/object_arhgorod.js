...
$('#arhgorod').on('click', loadComments); //запуск по кнопке
/* $('#load').ready(loadComments); */
	...
	let urlDomainArhgorod = "arhgorod";

	function loadComments() {
		sendRequest('wall.search', {/* owner_id: -10564356, */ domain: urlDomainArhgorod, count: 50, query: "МДУ", query: "Мезенское", query: "дороги", query: "мезенцы", query: "дорожники", query: "тротуар", v: 5.131}, function (data) {
...
/*                             + '<a target="_blank" href="https://vk.com/otkrytiiarkhangelsk?w=wall' + '-161193561' + '_' + f.post_id + '">' 
						...
						+ '<div class="comments__link">'
							+ '<a target="_blank" href="https://vk.com/' + urlDomainArhgorod + '?w=wall' + /* f.from_id */ f.owner_id + '_' + f.post_id + '">' 
							+ 'ссылка на пост' + '</a>' 
						+'</div>'
                + '</li>';
        }
